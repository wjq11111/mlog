			...
			$.ajax({
				   ...
				   success: function(msg){
					   $("#uul").tree({
							data:msg,
							onBeforeExpand:function(node,param){
								$('#uul').tree('options').url = '<%=request.getContextPath()%>/module/listJsonForMenu.action?id='+node.id
							},
							onClick : function(node){
								if(node.attributes.isend != 0 && node.attributes.isend != null && node.attributes.isend != 'null'){
									eoc.cms.main.openMainTab(node,id);
								}
							}
						});
